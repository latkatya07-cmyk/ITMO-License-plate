...
|:------------|:-------------|:---------|:-----------:|:-----------------:|:------|
| **Baseline** | `32×100`, `lr=1e-3`, `Adam`, 20 эпох | Стандартные гиперпараметры | `0.063` | `0.594` | Оптимальный базовый уровень |
| **Exp 1**    | `64×200`, `lr=1e-3`, `Adam`, 20 эпох | Увеличение разрешения входа в 4× | `0.205` | `0.178` | ❌ Сходимость замедлилась, переусложнение |
| **Exp 2**    | `32×100` + `ReduceLROnPlateau`, 20 эпох | Адаптивное уменьшение LR при plateau | `0.129` | `0.308` | ❌ LR не снижался, scheduler не понадобился |
| **Exp 3**    | `32×100`, аугментации, 20 эпох | Геометрические + фотометрические искажения | `[... ]` | `[... ]` | Ожидание: улучшение устойчивости к шуму/ракурсу |
