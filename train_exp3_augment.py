...
class CNNBackbone(nn.Module):
    def __init__(self, in_ch=1):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(in_ch, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU(inplace=True), nn.MaxPool2d(2, 2),
            nn.Conv2d(64, 128, 3, 1, 1), nn.BatchNorm2d(128), nn.ReLU(inplace=True), nn.MaxPool2d(2, 2),
            nn.Conv2d(128, 256, 3, 1, 1), nn.BatchNorm2d(256), nn.ReLU(inplace=True), nn.MaxPool2d((2, 2), (2, 1), (0, 1)),
            nn.Conv2d(256, 256, 3, 1, 1), nn.BatchNorm2d(256), nn.ReLU(inplace=True), nn.MaxPool2d((2, 2), (2, 1), (0, 1)),
        )
...
class CRNN(nn.Module):
    def __init__(self, img_h, num_channels, num_classes, hidden_size=256, num_layers=2):
        ...
        self.rnn = nn.LSTM(input_size=256, hidden_size=hidden_size, num_layers=num_layers,
...
class CRNNDataset(Dataset):
    def __init__(self, root: Path, split: str, char_to_idx: dict, transform=None):
    ...
    def _load_samples(self) -> List[Tuple[str, str]]:
        ...
        for jf in sorted(ann_dir.glob("*.json")):
            ...
            img_path = self.root / self.split / "img" / (jf.stem + ".png")
            if img_path.exists(): samples.append((str(img_path), text))
        return samples
...
def collate_fn(batch):
    ...
    return torch.stack(images), torch.cat(targets), torch.tensor(lengths, dtype=torch.long)

def load_alphabet(path: Path) -> Tuple[dict, dict]:
    with open(path, "r", encoding="utf-8") as f: chars = [l.strip() for l in f if l.strip()]
    c2i = {c: i for i, c in enumerate(chars)}
    return c2i, {i: c for i, c in enumerate(chars)}

def decode_sequence(pred_ids: torch.Tensor, idx_to_char: dict, blank_idx: int) -> str:
    text, prev = [], -1
    for idx in pred_ids.cpu().numpy():
        if idx != blank_idx and idx != prev: text.append(idx_to_char.get(idx, ""))
        prev = idx
    return "".join(text)

def compute_cer(pred: str, target: str) -> float:
    if not target: return float(len(pred) > 0)
...
def evaluate(model, loader, criterion, device, blank_idx, idx_to_char):
    ...
    for images, targets, lengths in loader:
        ...
        for pred_row, tgt in zip(pred_ids, targets.split(lengths.tolist())):
...
def main():
    ...
    val_transform = T.Compose([T.Resize((IMG_HEIGHT, IMG_WIDTH)), T.ToTensor()])

    train_ds = CRNNDataset(DATA_ROOT, "train", char_to_idx, transform=train_transform)
    val_ds = CRNNDataset(DATA_ROOT, "val", char_to_idx, transform=val_transform)
    ...
    print(f"[{EXP_NAME}] Device: {DEVICE} | Classes: {num_classes} | Aug: On")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), DATA_ROOT / f"crnn_{EXP_NAME}_best.pt")
        
        print(f"Epoch {epoch:2d} | Train: {train_loss:.4f} | Val: {val_loss:.4f} | CER: {val_cer:.4f} | Acc: {val_acc:.4f}")

    torch.save(model.state_dict(), DATA_ROOT / f"crnn_{EXP_NAME}_final.pt")
    print(f"✓ Saved: crnn_{EXP_NAME}_best.pt, crnn_{EXP_NAME}_final.pt")
